...
export function parseAnimalData(apiResult: any, originIds: number[] = []) {
  ...
  const rawColorVariants = extractColorVariants(apiResult.title, wikitext);
  ...
  return {
    ...
    enclosureSizes,
    origins: originIds.map((id) => ({ id })), // Behält manuelle Übergabe bei
    rawColorVariants,
    wikiOrigins: rawColorVariants.length > 0 ? rawColorVariants[0].obtainedFrom : [],
